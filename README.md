...
- `Refresh Problem` / `Refresh Problem List` / `Refresh Daily Challenge`
- `Open Code` / `Switch Language`
- `Test Solution` / `Submit Solution`
- `Clear Cache`

`leetdock.defaultLanguage` 保存默认语言。侧栏的刷新按钮会同时刷新题目列表和每日挑战。`Clear Cache` 会清理题目、每日挑战和最近记录缓存，但不会退出登录、修改默认语言、删除代码文件或重置代码目录。
